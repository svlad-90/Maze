...
using Maze_MathUtils;

namespace Maze_VectorUtils
{
    ...
    public static class VectorUtils
    {
        ...
        {
            if (!useRadians) a *= MathUtils.DEG_TO_RAD;
            if (yDominant)
            {
        ...
        {
            input.x = MathUtils.Clamp(input.x, max.x, min.x);
            input.y = MathUtils.Clamp(input.y, max.y, min.y);
            input.z = MathUtils.Clamp(input.z, max.z, min.z);
            return input;
        }
        ...
        {
            return Mathf.Atan2(v.y, v.x) * MathUtils.RAD_TO_DEG;
        }
        ...
        {
            if (!bUseRadians) a *= MathUtils.DEG_TO_RAD;
        ...
        public static Vector2 RotateToward(Vector2 from, Vector2 to, float a, bool bUseRadians = false)
        {
            ...
            a2 = MathUtils.ShortenAngleToAnother(a2, a1, true);
        ...
        {
            var a = MathUtils.NormalizeAngle(Mathf.Lerp(Mathf.Atan2(from.y, from.x), Mathf.Atan2(to.y, to.x), t), true);
        ...
        {
            // // Due to float error the dot / mag can sometimes be ever so slightly over 1, which can cause NaN in acos.
            //return Mathf.Acos(Vector3.Dot(a, b) / (a.magnitude * b.magnitude)) * MathUtils.RAD_TO_DEG;
            double d = System.Math.Sqrt((double)a.sqrMagnitude * (double)b.sqrMagnitude);
            if (d < MathUtils.DBL_EPSILON) return 0f;
        ...
        public static float AngleOffAroundAxis(Vector3 v, Vector3 forward, Vector3 axis, bool clockwise = false)
        {
            ...
            return Mathf.Atan2(Vector3.Dot(v, right), Vector3.Dot(v, forward)) * MathUtils.RAD_TO_DEG;
        }
        ...
        {
            if (bUseRadians) a *= MathUtils.RAD_TO_DEG;
            Quaternion q;
            if (clockwise)
        ...
        /// <returns></returns>
        public static bool NearSameAxis(Vector3 a, Vector3 b, float epsilon = MathUtils.EPSILON)
        {
            return MathUtils.FuzzyEqual(Mathf.Abs(Vector3.Dot(a.normalized, b.normalized)), 1.0f, epsilon);
        }

        public static bool NearZeroVector(this Vector4 v)
        {
            return MathUtils.FuzzyEqual(v.sqrMagnitude, 0f, MathUtils.EPSILON_SQR);
        }
        ...
        {
            return MathUtils.FuzzyEqual(Vector3.SqrMagnitude(a - b), 0f, MathUtils.EPSILON_SQR);
        }

        public static bool FuzzyEquals(this Vector2 a, Vector2 b, float epsilon)
        {
            return MathUtils.FuzzyEqual(Vector3.SqrMagnitude(a - b), 0f, epsilon);
        }
        ...
        {
            return MathUtils.FuzzyEqual(Vector4.SqrMagnitude(a - b), 0f, MathUtils.EPSILON_SQR);
        }

        public static bool FuzzyEquals(this Vector4 a, Vector4 b, float epsilon)
        {
            return MathUtils.FuzzyEqual(Vector4.SqrMagnitude(a - b), 0f, epsilon);
        }
